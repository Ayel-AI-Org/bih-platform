import { TERMS_AND_CONDITIONS_TEXT } from "@/config/legalContent";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { Scale } from "lucide-react";

export const TermsPage = () => {
  const data = TERMS_AND_CONDITIONS_TEXT;
          ...
          <CardContent className="pt-6 space-y-6 text-sm text-slate-700 leading-relaxed font-sans">
            ...
            {data.sections.map((section, idx) => (
              <div key={idx} className="space-y-3">
                ...
                {section.bullets && (
                  <ul className="space-y-3 pl-4 mt-2">
                    {section.bullets.map((bullet, bIdx) => (
                      <li key={bIdx} className="list-disc pl-1">
                        <strong className="text-slate-800">{bullet.role}: </strong>
                        {bullet.text}
                      </li>
                    ))}
                  </ul>
